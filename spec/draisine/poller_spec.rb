...
describe Draisine::Poller do
  ...
  describe "#poll" do
    ...
    it "imports new records" do
      ...
      expect {
        subject.poll(start_date, end_date, import_created: true)
    ...
    it "imports updated records" do
      ...
      expect {
        subject.poll(start_date, end_date, import_updated: true)
    ...
    it "doesn't import updated records when flag is not set" do
      ...
      expect {
        subject.poll(start_date, end_date, import_updated: false)
      }.not_to change { model_class.count }
      lead.reload
    ...
    it "removes deleted records" do
      ...
      expect {
        subject.poll(start_date, end_date, import_deleted: true)
      }.to change { model_class.count }.by(-1)
    end
    ...
    it "doesn't remove deleted records when flag is not set" do
      ...
      allow(sf_client).to receive(:get_deleted_ids).and_return(["A001"])
      expect {
        subject.poll(start_date, end_date, import_deleted: false)
      }.to change { model_class.count }.by(0)
    end

    it "supports a different system modstamp polling mechanism" do
      subject = described_class.new(model_class, :system_modstamp)
      ...
      expect { subject.poll(start_date, end_date) }.to change { model_class.count }.by(1)
    end
    ...
    describe "results" do
      ...
      it "returns salesforce and db counters after running" do
        ...
        result = subject.poll(start_date, end_date)
      ...
      it "returns a number of salesforce created, updated and deleted records" do
        ...
        result = subject.poll(start_date, end_date, import_created: true, import_updated: true, import_deleted: true)
